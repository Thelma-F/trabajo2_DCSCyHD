...
#### Palabras más frecuentes de todos los medios
...
A mejorar:
        ...
        - cambiar las referencias del eje x para que quede algo similar a "frecuencia del término cada 1000 palabras"
...
### Gráfico 2: palabras más características del medio (tf_idf)

A mejorar:
        - que las barras representen los valores de tf_idf de los términos
        - ordenar por orden descendente las palabras
        - cambiar las referencias del eje x para que quede algo similar a "frecuencia del término cada 1000 palabras"
...
millones, gobierno, país, argentina, dólares, acuerdo, presidente, Trump, unidos, mercado, ciento, pesos, dólar, banco, economía
...
ciudad, personas, Aires, Argentina, país, agua, mundo, productos, nacional, san, zona, centro, sistema, metros, forma

- Tópico 7: Deportes y Entretenimiento
equipo, partido, final, Argentina, Boca, copa, fútbol, club, River, mundial, argentino, selección, minutos, jugadores, mundo


```{r principales_noticias_x_topico, eval=FALSE, include=FALSE}

# Este código no se corre, es sólo para revisar qué noticias pertenecen a qué tópico
...
### Tópicos resultantes:
      - Topico 1: política/elecciones
      - Topico 2: politica exterior
      - Topico 3: Economía
      - Topico 4: dudoso? FREX: horóscopo, armstrong, astronautas, bibliotecas, binz, piscis, quimioterapia hacer chequeo
      - Topico 5: justicia y seguridad
      - Topico 6: espectaculo
      - Topico 7: deportes
...
```{r noticias_topico_5, eval=FALSE, include=FALSE}
...
```
    ...
    ESTOS BLOQUES NO SE CORREN CUANDO SE CORRE TODO EL CÓDIGO
---------
...
# Obtener una muestra del 10%
corpus_topic_5_muestra <- corpus_topic_5 %>%
  sample_frac(0.5, replace = FALSE)

# Ahora corpus_topic_5_muestra contiene el 50% aleatorio de los datos originales
...
```{r elimina_objetos}
# Como ya no se utilizan e implican mucho peso, se elimina el embedding
...
Por ese motivo, es necesario un balanceo de medios y orientaciones antes de continuar con el análisis. Considerando que sólo existen 2 diarios de orientación "progresista" (mientras el resto de las orientaciones poseen 3 diarios distintos) y uno de ellos (Minuto1) sólo posee 56 noticias, utilizaremos ese valor como base. De esta forma, dentro de la orientación "progresista" habrá 56 noticias de ese diario y otras 56 de Página12, dando un total de 112 noticias.
...
noticias_split <- rsample::initial_split(corpus_c_embedding,
                                         strata = medio)
