...
##PRUEBA 1	RANDOM FOREST	CORPUS CON EXCLUSIÓN DE ORIENTACIÓN "NEUTRO" (Se eliminan al inicio). Sin variable “medio”, Sin balancear en la receta por orientación. (Para ver si overfitea menos)	
...
corpus_noticias_t5 <- noticias_topico_5 |>
  rename(titulo = document) |> 
  left_join(noticias) |>
  select(titulo, gamma, id, orientacion, texto) |> #para probar sin la variable medio
  #select(titulo, gamma, id, medio, orientacion, texto) |> 
  mutate(orientacion = gsub("\\+ ", "", orientacion)) |>
...
```
#Comentario Beti: todo este bloque no lo use, lo dejo por las dudas que lo necesites, sino se puede eliminar.
...
# Como se saltea el paso anterior, se carga el modelo que ya está guardado en la carpeta de "models"  VER PORQUE SE SACO ESTA LINEA DEL SCRIPT GENERLA
...
metrics_rf <- collect_metrics(tune_params) 
print (metrics_rf)
```
...
##PPRUEBA 2	RANDOM FOREST	CORPUS CON EXCLUSIÓN DE ORIENTACIÓN "NEUTRO" 
Con variable medio, sin eliminarla.  Balanceando en la receta por orientación	
...
# Recopilar métricas en un tibble
metrics_rf <- collect_metrics(tune_params_rf) 
print (metrics_rf)
```
...
##PRUEBA 3	REGRESION LOGISTICA LASSO	CORPUS CON EXCLUSIÓN DE ORIENTACIÓN "NEUTRO. Sin balancear en la receta por orientación 
...
`
###PRUEBA 4	RANDOM FOREST	Corpus con variable Orientación DICOTÓMICA
CONSERVADOR / NO CONSERVADOR (incluye neutro y progresista)
...
# Recopilar métricas en un tibble
metrics_rf <- collect_metrics(tune_params2) 
...
bind_rows(f_meas(preds_embed, orientacion, .pred_class))
```
###PRUEBA 5	REGRESION LOGISTICA LASSO	Corpus con variable Orientación DICOTÓMICA
CONSERVADOR / NO CONSERVADOR (incluye neutro y progresista)
Balanceando en la receta por orientación	
