...
# PRUEBA 2 RANDOM FOREST
    - Excluye de la variable orientación la categoría "neutro" 
    - Con variable medio
...
# Establecimiento del modelo a tunear
modelo_a_tunear <- rand_forest(trees = 1000,
                               mtry = tune(),
                               min_n = tune()) %>%
  set_mode("classification") %>%
...
Es claro que no existe un balanceo de orientaciones: "conservador" y "neutro" poseen casi un 40% de noticias, mientras "progresista" sólo posee un 21%. Lo mismo puede decirse sobre la orientación de los diarios.  Con respecto a la categoría "conservador", Clarín tiene el cuádruple de casos que Infobae; lo mismo Perfil respecto a Crónica o Télam en "neutro"; y Página12 posee más del doble de noticias que Minuto1
...
# GRÁFICO 8: COMPARANDO MÉTRICAS DE LOS MODELOS PROBADOS (RANDOM FOREST Y REGRESIÓN LOGÍSTICA REGULARIZADA POR LASSO)
...
grafico8 <- metricas_modelos |> 
  mutate(.metric = case_when(
    .metric == "accuracy" ~ "Accuracy",
    .metric == "roc_auc" ~ "Curva ROC",
    .metric == "precision" ~ "Precision",
    .metric == "recall" ~ "Recall",
    .metric == "f_meas" ~ "F_meas",
    TRUE ~ as.character(.metric))) |> 
  mutate(modelo = case_when(
    modelo == "modelo_1" ~ "Prueba 1",
    modelo == "modelo_2" ~ "Prueba 2",
    modelo == "modelo_3" ~ "Prueba 3",
    modelo == "modelo_4" ~ "Prueba 4",
    modelo == "modelo_5" ~ "Prueba 5",
    modelo == "modelo_6" ~ "Prueba 6",
    TRUE ~ as.character(modelo))) |> 
  ggplot(aes(x = .metric, y = .estimate, fill = modelo)) +
  geom_col(position = "dodge") +
  scale_fill_viridis_d() +
  labs(title = "Principales métricas según modelo", 
       caption = "Gráfico 8",
       plot.title = element_text(size = 10),
       x = "Métricas", 
       y = "Estimado") +
  guides(fill = guide_legend(title = "Prueba")) +
  theme_minimal()
...
# Guardar el gráfico como un archivo
ggsave(filename = "Graficos/Grafico8.png", 
       plot = grafico8, 
       width = 8, height = 6)
