...
Vamos a usar un embedding para predecir la orientación del diario en función del texto de la noticia. En particular, éste está entrenado mediante el algoritmo wor2vec por C. Cardelino. Está entrenado sobre un corpus grande en español. Más detalles [aquí](https://crscardellino.ar/SBWCE/).
...
```{r combinaciones_hiperparametros, eval=FALSE, include=FALSE}
tictoc::tic()
...
best_ROC <- select_best(a_tunear_4, "roc_auc")
...
noticias_c_embedding_lasso <- noticias_c_embedding
## Split
set.seed(1234)
noticias_split <- initial_split(noticias_c_embedding_lasso, strata = orientacion)
...
final_params_lasso_embed <- finalize_workflow(wf, chosen_auc_embed)
...
best_ROC <- select_best(a_tunear_6, "roc_auc")
