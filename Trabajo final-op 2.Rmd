...
grafico3 <- top_palabras_topicos %>%
  ...
  labs(x = "Probabilidad de aparición en una noticia cada 1000 palabras", 
       y = "Términos") +
  theme_minimal() +
  theme(panel.grid = element_blank(),  # Eliminamos las líneas del área
        panel.border = element_blank(), # Eliminamos las líneas del área
        plot.background = element_rect(fill = "white"),
        axis.text = element_text(face = "bold")) +  # Establecemos el texto del eje en negrita
  labs(title = "Principales palabras por tópicos según modelo LDA",
       caption = "Gráfico 3",
       plot.title = element_text(size = 10))

# Guardamos el gráfico en la carpeta "Graficos"
ggsave(filename = "Graficos/Grafico3.png", plot = grafico3, width = 8, height = 6)
...
Para observar qué títulos aparecen en las noticias del tópico x


```{r top_noticias_x_topico, eval=FALSE, include=FALSE}

# Este código no se corre, es sólo para revisar qué noticias pertenecen más probablemente a un tópico (ej: tópico 1)
...
```{r grafico 4- no incluido en informe}
...
grafico4 <- ggplot(topicos_por_medio, aes(x = factor(medio), y = mean)) +
...
### Tópicos resultantes LDA  (REVISAR)
Tópico 1: Policiales
...
### Gráfico 6: Principales palabras por tópico según medio en STM /VER SI ESTE TITULO ES CORRECTO
...
theta_stm_7 %>%
  ...
  ggplot(aes(x=medio, y=mean, fill=medio)) + 
    ...
    facet_wrap(~case_when(
           ...
           topic == 6 ~ "6.Cultura y entretenimiento",
           topic == 7 ~ "7.Deportes",
           TRUE ~ as.character(topic)), scales = "free_y", strip.position = "bottom") + 
    theme_minimal() +
    ...
    geom_text(aes(label = medio), vjust = -0.5, size = 3, position = position_dodge(width = 1))
...
## PRUEBA 1	RANDOM FOREST:
    ...
    - Sin variable “medio” (en la fusión si lo pongo en la receta luego me da un error en el tuneo NO SE POR QUE)
    - Sin balancear en la receta por orientación (para ver si overfitea menos)	
**Curva Roc Final:  0.6690901 ** (empeoro en relacion al anterior 0.75)
...
## Split
set.seed(1234)
noticias_split <- initial_split(noticias_c_embedding, strata = orientacion)
...
## PRUEBA 3	REGRESION LOGISTICA LASSO:
    - Con exclusión de orientación "neutro"
    - Sin balancear en la receta por orientación 
**Curva Roc Final 0.6206213 (Empeora antes era de 0.77)


```{r A CONFIRMAR}

# Se arma el objeto "corpus_noticias_t6", que con el conjunto de noticias que más probablemente pertenezcan al tópico  (política/ elecciones), a lo cual se suman el texto, el medio y la orientación del medio del cual sale la noticia
...
```{r cars}
corpus_Topic6_lasso <- corpus_noticias_t6 
# Verificar los cambios
table(corpus_Topic6_lasso$orientacion)
...
noticias_embed_lasso <- corpus_Topic6_lasso %>%
        mutate(texto = str_replace_all(texto, "'\\[.*?¿\\]\\%'", " ")) %>%
        mutate(texto = str_replace_all(texto, "[[:digit:]]+", "DIGITO"))
...
metricas_3 <- roc_auc(preds_embed_lasso, orientacion, .pred_conservador) %>%
...
Carga del embedding. Vamos a usar un embedding entrenado mediante el algoritmo wor2vec por C. Cardelino. Está entrenado sobre un corpus grande en español. Pueden encontrar los detalles [aquí](https://crscardellino.ar/SBWCE/).
...
tune_params <- read_rds('models/tune_params2.rds')
...
# Calcular las métricas de evaluación
metricas_5 <- roc_auc(preds_embed_lasso, orientacion, .pred_conservador) %>%
  bind_rows(accuracy(preds_embed_lasso, orientacion, .pred_class)) %>%
  bind_rows(precision(preds_embed_lasso, orientacion, .pred_class)) %>%
  bind_rows(recall(preds_embed_lasso, orientacion, .pred_class)) %>%
  bind_rows(f_meas(preds_embed_lasso, orientacion, .pred_class)) |> 
...
#objetos_a_no_eliminar <- setdiff(all_objects <- ls(), 
                                 c("metricas_1", "metricas_2", "metricas_3", 
                                   "metricas_4", "metricas_5", "metricas_6")

rm(list = objetos_a_no_eliminar)
