...
#Paso la primera letra de los medios en mayúsculas
...
# Crear el gráfico (BS: Agregué titulo y saque la leyenda)

ggplot(top_words_by_medio, aes(x = tf, 
                               y = reorder(word, tf), 
                               fill = medio)) +
  geom_col() +
  scale_fill_discrete(name = NULL) +  # Eliminar la leyenda
  labs(x = "TF", 
       y = "Palabra",
       title = "Palabras más frecuentes por medio") +  # Agregar el título
...
### Opción 2: Gráfico con palabras más características del medio (tf_idf) ver este grafico

```{r grafico_palabras_características}

# Crear el gráfico #VER ESTE GRAFICO
...
top_palabras_topicos %>%
  ...
  scale_y_reordered() +
  theme_minimal()+
  labs(title = "Principales palabras por Tópico según modelo LDA",
...
Tópico 2: Vida y Relaciones Personales (vida, casa, gente, historia, mujer, familia, mundo, amor, vídeo, película, programa, música, hijo, padre)
Tópico 3: Sociedad y Bienestar (mujeres, personas, salud, vida, argentina, mundo, social, educación, país, sociedad, sistema, sociales, política, forma, nacional)
...
topicos_noticias %>%
  ...
  ggplot() +
    ...
    labs(title = "Tópicos por medio")  # Establecer el título del gráfico
...
#Tópico 3: Economía
Tópico 4: ¿dudoso? FREX: horóscopo, armstrong, astronautas, bibliotecas, binz, piscis, quimioterapia hacer chequeo
...
#VEr estos tópicos. Hacer comparacion entre LDA y STM clase de modulo 5
