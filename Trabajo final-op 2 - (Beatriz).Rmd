...
library(stm)
library(reshape2)
#AGREGAR ESTAS LIBERÍAS AL SCRIPT GENERAL
library(textrecipes)
library(textclean)
<<<<<<< HEAD
...
library(glmnet)
stop
>>>>>>> 5727b8416890539435011b4bb7a149fd4c338dd4
```
...
#Ver que en el corpus hay noticias que pertenecen a más de un tópico, se resuelve cuando se filtra por un tópico
#Se observa que, entonces, el input que vamos a utilizar será el campo `texto` y la variable dependiente `orientacion`, la cual debe ser trabajada como factor, se eliminan los caracter +-.
...
#base_para_modelar <- base_juguete %>% 
  #mutate(realiza_trabajo_domestico = factor(realiza_trabajo_domestico,levels = c("Si","No")
  #                                          )))

corpus_topic <- topicos_noticias %>%
  rename(titulo = document) %>%
  left_join(
    noticias %>%
...
```{r}
corpus_topic_cuenta <- corpus_topic %>%
  count(topic) %>%
  mutate(
    porcentaje_absoluto = n,
...
labelTopics(stm_7)
```
Topico1: política/elecciones
Topico2: politica exterior
Topico 3: Economía
...
```{r cars}
corpus_Topic5_lasso <- corpus_Topic5_muestra %>%
